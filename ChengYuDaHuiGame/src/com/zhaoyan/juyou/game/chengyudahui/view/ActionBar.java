...
public class ActionBar extends LinearLayout {
    ...
    public ActionBar(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.zyActionBarStyle);
    }
    ...
    public ActionBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTitle = a.getString(R.styleable.ActionBar_ab_title);
        ...
        if (mHomeDrawable == null) {
        	int appIconResId = getContext().getApplicationInfo().icon;
            mHomeDrawable = getResources().getDrawable(appIconResId);
        }
        ...
        switch (type) {
            case 2:
                mType = Type.Empty;
                layoutID = R.layout.zy_action_bar_empty;
                break;
            case 1:
                mType = Type.Dashboard;
                layoutID = R.layout.zy_action_bar_dashboard;
                break;
            case 0:
            default:
                mType = Type.Normal;
                layoutID = R.layout.zy_action_bar_normal;
                break;
        }
    ...
    protected void onFinishInflate() {
        ...
        if (!mMerging) {
            ...
            switch (mType) {
                case Dashboard:
                	mHomeView = (ImageView) findViewById(R.id.zy_action_bar_home_item);
                	mHomeView.setImageDrawable(mHomeDrawable);
                	mTitleView = (TextView) findViewById(R.id.zy_action_bar_title);
                    setTitle(mTitle);
                    break;
                ...
                case Normal:
                default:
                    mHomeButton = (ImageButton) findViewById(R.id.zy_action_bar_home_item);
                    mHomeButton.setOnClickListener(mClickHandler);
                    mHomeButton.setImageDrawable(mHomeDrawable);
                    mHomeButton.setContentDescription("Home");
                    mTitleView = (TextView) findViewById(R.id.zy_action_bar_title);
                    setTitle(mTitle);
                    break;
	...
	public void setActionHomeAsUpEnable(boolean enable) {
		switch (mType) {
		case Dashboard:
		case Empty:
			break;
		case Normal:
			mHomeButton.setImageResource(R.drawable.ic_action_back);
			break;
		default:
			break;
		}
    ...
    public void setActionHomeEnable(boolean enable){
    	if(!enable){
    		setType(Type.Empty);
    	} 
    }
...
//                addView(divider);
            ...
            itemView.findViewById(R.id.zy_action_bar_item).setOnClickListener(mClickHandler);

            final int size = (int) getResources().getDimension(R.dimen.zy_action_bar_height);
        ...
        public void onClick(View v) {
            if (mOnActionBarListener != null) {
                ...
                for (int i = 0; i < itemCount; i++) {
                    ...
                    final View itemButton = item.getItemView().findViewById(R.id.zy_action_bar_item);
                    if (v == itemButton) {
                        item.onItemClicked();
