...
public class MessageChatAdapter extends BaseListAdapter<ZhaoYanMsg> {
	...
	private View createViewByType(ZhaoYanMsg message, int position) {
		int type = message.getMsgType();
	   if(type==MsgConfig.TYPE_IMAGE){//图片类型
		...
		}else if(type==MsgConfig.TYPE_LOCATION){//位置类型
		...
		}else if(type==MsgConfig.TYPE_VOICE){//语音类型
	...
	public View bindView(final int position, View convertView, ViewGroup parent) {
		...
		final ZhaoYanMsg item = list.get(position);
		if (convertView == null) {
...
//				||getItemViewType(position)==TYPE_SEND_IMAGE//图片单独处理
			...
			//状态描述
			if(item.getStatus()==MsgConfig.STATUS_SEND_SUCCESS){//发送成功
				progress_load.setVisibility(View.INVISIBLE);
				iv_fail_resend.setVisibility(View.INVISIBLE);
				if(item.getMsgType()==MsgConfig.TYPE_VOICE){
			...
			}else if(item.getStatus()==MsgConfig.STATUS_SEND_FAIL){//服务器无响应或者查询失败等原因造成的发送失败，均需要重发
				progress_load.setVisibility(View.INVISIBLE);
			...
			}else if(item.getStatus()==MsgConfig.STATUS_SEND_RECEIVERED){//对方已接收到
				progress_load.setVisibility(View.INVISIBLE);
				iv_fail_resend.setVisibility(View.INVISIBLE);
				if(item.getMsgType()==MsgConfig.TYPE_VOICE){
					tv_send_status.setVisibility(View.GONE);
			...
			}else if(item.getStatus()==MsgConfig.STATUS_SEND_START){//开始上传
...
//						mContext.startActivity(intent);
		...
		case MsgConfig.TYPE_LOCATION://位置信息
			try {
...
//							mContext.startActivity(intent);
			...
			break;
		case MsgConfig.TYPE_VOICE://语音消息
			try {
				if (text != null && !text.equals("")) {
					tv_voice_length.setVisibility(View.VISIBLE);
					String content = item.getContent();
					if (item.getBelongId().equals(currentObjectId)) {//发送的消息
						if(item.getStatus()==MsgConfig.STATUS_SEND_RECEIVERED
								||item.getStatus()==MsgConfig.STATUS_SEND_SUCCESS){//当发送成功或者发送已阅读的时候，则显示语音长度
							tv_voice_length.setVisibility(View.VISIBLE);
						...
						boolean isExists = true;//判断一下录音文件本地是否存在
...
//				iv_voice.setOnClickListener(new NewRecordPlayClickListener(mContext,item,iv_voice));
	...
	/** 获取图片的地址--
	  ...
	  * @throws
	  */
	private String getImageUrl(ZhaoYanMsg item){
		String showUrl = "";
	...
	/** 处理图片
	  ...
	  * @throws
	  */
	private void dealWithImage(int position,final ProgressBar progress_load,ImageView iv_fail_resend,TextView tv_send_status,ImageView iv_picture,ZhaoYanMsg item){
		String text = item.getContent();
		if(getItemViewType(position)==TYPE_SEND_IMAGE){//发送的消息
			if(item.getStatus()==MsgConfig.STATUS_SEND_START){
				progress_load.setVisibility(View.VISIBLE);
				iv_fail_resend.setVisibility(View.INVISIBLE);
				tv_send_status.setVisibility(View.INVISIBLE);
			}else if(item.getStatus()==MsgConfig.STATUS_SEND_SUCCESS){
				progress_load.setVisibility(View.INVISIBLE);
				iv_fail_resend.setVisibility(View.INVISIBLE);
				tv_send_status.setVisibility(View.VISIBLE);
				tv_send_status.setText("已发送");
			}else if(item.getStatus()==MsgConfig.STATUS_SEND_FAIL){
				progress_load.setVisibility(View.INVISIBLE);
				iv_fail_resend.setVisibility(View.VISIBLE);
				tv_send_status.setVisibility(View.INVISIBLE);
			}else if(item.getStatus()==MsgConfig.STATUS_SEND_RECEIVERED){
				progress_load.setVisibility(View.INVISIBLE);
				iv_fail_resend.setVisibility(View.INVISIBLE);
				tv_send_status.setVisibility(View.VISIBLE);
				tv_send_status.setText("已阅读");
